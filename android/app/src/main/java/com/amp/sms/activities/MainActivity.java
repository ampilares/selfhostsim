package com.amp.sms.activities;
...
import com.amp.sms.ApiManager;
import com.amp.sms.AppConstants;
import com.amp.sms.BuildConfig;
import com.amp.sms.selhostsimUtils;
import com.amp.sms.R;
import com.amp.sms.dtos.RegisterDeviceInputDTO;
import com.amp.sms.dtos.RegisterDeviceResponseDTO;
import com.amp.sms.helpers.SharedPreferenceHelper;
import com.amp.sms.helpers.VersionTracker;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (gatewayEnabled && stickyNotificationEnabled) {
            selhostsimUtils.startStickyNotificationService(mContext);
        ...
        String[] missingPermissions = Arrays.stream(AppConstants.requiredPermissions).filter(permission -> !selhostsimUtils.isPermissionGranted(mContext, permission)).toArray(String[]::new);
...
//        selhostsimUtils.startStickyNotificationService(mContext);
            ...
            apiCall.enqueue(new Callback<RegisterDeviceResponseDTO>() {
                ...
                public void onResponse(Call<RegisterDeviceResponseDTO> call, Response<RegisterDeviceResponseDTO> response) {
                    ...
                    if (enabled) {
                        ...
                        selhostsimUtils.stopStickyNotificationService(mContext);
                    }
                    compoundButton.setEnabled(true);
                }
                @Override
                public void onFailure(Call<RegisterDeviceResponseDTO> call, Throwable t) {
                    ...
                    selhostsimUtils.logException(t, "Error updating device");
                    compoundButton.setEnabled(true);
                }
            });
        });
        ...
        scanQRBtn.setOnClickListener(view -> {
            ...
            intentIntegrator.setPrompt("Contact administrator to request API Key for device registration.");
    ...
    private void renderAvailableSimOptions() {
        try {
            ...
            selhostsimUtils.getAvailableSimSlots(mContext).forEach(subscriptionInfo -> {
    ...
    public void onRequestPermissionsResult(int requestCode, String permissions[], int[] grantResults) {
        ...
        boolean allPermissionsGranted = Arrays.stream(permissions).allMatch(permission -> selhostsimUtils.isPermissionGranted(mContext, permission));
                    ...
                    if (!deviceIdInput.isEmpty()) {
                        ...
                        apiCall.enqueue(new Callback<RegisterDeviceResponseDTO>() {
                            ...
                            public void onFailure(Call<RegisterDeviceResponseDTO> call, Throwable t) {
                                ...
                                Log.e(TAG, "API_ERROR "+ t.getMessage());
                                Log.e(TAG, "API_ERROR "+ t.getLocalizedMessage());
                                selhostsimUtils.logException(t, "Error registering device");
                                registerDeviceBtn.setEnabled(true);
                                registerDeviceBtn.setText("Update");
                            }
                        });
    ...
    private void handleRequestPermissions(View view) {
        boolean allPermissionsGranted = Arrays.stream(AppConstants.requiredPermissions).allMatch(permission -> selhostsimUtils.isPermissionGranted(mContext, permission));
        if (allPermissionsGranted) {
            Snackbar.make(view, "Already got permissions", Snackbar.LENGTH_SHORT).show();
            return;
        }
        String[] permissionsToRequest = Arrays.stream(AppConstants.requiredPermissions).filter(permission -> !selhostsimUtils.isPermissionGranted(mContext, permission)).toArray(String[]::new);
